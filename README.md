...
- Resolves uploaded files through `getFile` and serves bytes through both `GET /_sim/file/{id}`
  and the Telegram-style `GET /file/bot<TOKEN>/<file_path>` path, so SDK download flows work.
...
response. In remote mode, `setWebhook` also rejects URLs that resolve to private or loopback
addresses (an SSRF guard); pass `--allow-private-webhooks` to override for trusted internal targets.

Still, prefer running remote mode behind Tailscale, Cloudflare Tunnel, Caddy, or another HTTPS
reverse proxy. Examples are available in `deploy/systemd/` and `deploy/caddy/`.
